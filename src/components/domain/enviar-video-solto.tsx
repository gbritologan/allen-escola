...
/**
 * ENVIAR UM VÍDEO QUE NÃO É AULA.
 *
 * Teaser de curso, boas-vindas da Home — vídeos que existem sozinhos, sem
 * progresso, sem posição salva, sem conclusão. O que muda entre eles é só
 * QUAL ação preparar e remover; então o componente recebe as duas e não sabe
 * de curso nem de Home.
 *
 * Escolher já envia, como na capa (D-76): o segundo clique que ninguém vê é o
 * que faz upload nenhum acontecer.
 *
 * O arquivo vai do navegador direto ao provedor, por TUS — retomável, e sem
 * atravessar o nosso servidor, que tem teto de poucos megabytes (D-78).
 */
export function EnviarVideoSolto({
  titulo,
  temVideo,
  preparar,
  remover,
  ...
  /** Vira o nome do arquivo no provedor. Ajuda a achar lá dentro. */
  titulo: string
  temVideo: boolean
  preparar: () => Promise<Ticket>
  remover: () => Promise<void>
  ajuda: string
}) {
  const router = useRouter()
  const [etapa, setEtapa] = useState<Etapa>(temVideo ? { nome: 'pronto' } : { nome: 'parado' })
  ...
  async function enviar(arquivo: File | undefined) {
    ...
    new Upload(arquivo, {
      ...
      metadata: { filetype: arquivo.type, title: titulo },
    ...
    <div className="flex flex-col gap-3">
      ...
      {etapa.nome === 'pronto' ? (
        <div className="flex flex-wrap items-center gap-3 rounded-[var(--radius-control)] border border-line px-4 py-3">
          <span className="flex-1 text-caption text-ink-2">Vídeo no ar.</span>
          ...
          {ocupado ? (
            <>
              <span className="text-body text-ink-2">
                {etapa.nome === 'preparando' ? 'Preparando…' : `Enviando… ${etapa.progresso}%`}
              </span>
              {etapa.nome === 'enviando' && (
            ...
            <>
              <span className="text-body text-ink-2">
                {arrastando ? 'Solte aqui' : 'Arraste o vídeo'}
              </span>
              <button
                type="button"
              ...
              <span className="max-w-[46ch] text-caption text-ink-4">{ajuda}</span>
            </>
          )}
        </div>
      )}
